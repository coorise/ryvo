...
export function BrandLogo({ subtitle = "Mobility", href = "/landing", className }: BrandLogoProps) {
  const inner = (
    <>
  ...
  return (
    <Link href={href} className={cn("flex items-center gap-2 transition active:scale-95", className)}>
      {inner}
    </Link>
  );
}
